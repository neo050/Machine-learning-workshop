...
# Handle rare labels in 'Ethnicity' before encoding
...
# Now encode 'Gender' and the transformed 'Ethnicity' with pd.get_dummies
df_encoded = pd.get_dummies(df, columns=['Gender', 'Ethnicity'])

# Impute missing ESTIMATE values before feature selection
...
X = df_encoded.select_dtypes(include=[np.number]).drop(['ESTIMATE'], axis=1)
y = df['ESTIMATE']
